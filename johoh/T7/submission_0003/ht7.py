...
class Valintaikkuna:
    ...
    def custom(self):
        ...
        if self.__custom is False:
            self.__lopeta.forget()
            self.__tyhjä.forget()
            self.__korkeuslabel.pack(fill=X)
            self.__korkeusentry.pack(fill=X)
            self.__leveyslabel.pack(fill=X)
            self.__leveysentry.pack(fill=X)
            self.__miinojalabel.pack(fill=X)
            self.__miinojaentry.pack(fill=X)
            ...
            self.__korkeusentry.forget()
            self.__leveyslabel.forget()
            self.__leveysentry.forget()
            self.__miinojalabel.forget()
            self.__miinojaentry.forget()
    ...
    def customaloita(self):
        ...
        self.__korkeus = self.__korkeusentry.get()
        self.__leveys = self.__leveysentry.get()
        self.__miinoja = self.__miinojaentry.get()
        try:
            k = int(self.__korkeus)
            l = int(self.__leveys)
            m = int(self.__miinoja)
            if m > (l-1)*(k-1):
...
class Miinaharava:
    ...
    def __init__(self, x, y, m):
        ...
        self.__jäljellä = m
        self.__lippu = False
        self.__buttonit = [[None for i in range(x)]for j in range(y)]
        for i in range(y):
            for j in range(x):
        ...
        for sarake in self.__buttonit:
            for button in sarake:
                button.configure(width=2, height=1,
                                 ...
                                 font="Arial 12 bold",
                                 command=lambda
                                 a=self.__buttonit.index(sarake),
                                 b=sarake.index(button): self.aloitus(a,b))
            ...
            for sarake in self.__buttonit:
                for button in sarake:
                    button.configure(command=lambda
                                     a=self.__buttonit.index(sarake),
                                     b=sarake.index(button):
                                     self.oikeapainallus(a, b))
    ...
    def painallus(self, x, y):
        ...
        if (self.__buttonit[x][y].configure("relief")[-1] == SUNKEN and
                (self.__buttonit[x][y].configure("text")[-1] ==
                 str(self.laskemerkatut(x, y))or
                 self.__buttonit[x][y].configure("text")[-1] == " ")):
                    ...
                    if self.__lauta[i][j] == "X":
                        if self.__buttonit[i][j].configure("text")[-1] == "  ":
                            pass
                                ...
                                "bg")[-1] != "#FF4040":
                            self.__buttonit[i][j].configure(
                                image=self.__miinakuva,
                                height=24, width=22,
